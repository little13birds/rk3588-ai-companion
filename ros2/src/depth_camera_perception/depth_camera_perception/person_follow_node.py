...
from .depth_utils import estimate_bbox_depth_m
from .person_detector import Detection, create_person_detector
from .person_follow import FollowOutput, PersonFollowConfig, PersonFollowController
from .person_seek import PersonTarget
from .person_seek_node import (
    ReusableThreadingHTTPServer,
    _FpsCounter,
    _obstacle_bypass_active as _seek_obstacle_bypass_active,
    _rounded_or_none,
    _select_target,
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .badge.warn { background: rgba(255, 211, 106, 0.16); color: var(--yellow); }
    .badge.danger { background: rgba(255, 118, 117, 0.16); color: var(--red); }
    dl { margin: 22px 0 0; }
    .row {
      display: flex;
...
<body>
  <main>
    <section class="view"><img src="/stream.mjpg" alt="person follow stream"></section>
    <aside class="side">
      <h1>&#28145;&#24230;&#36319;&#38543;&#30417;&#25511;</h1>
      ...
      <dl>
        ...
        <div class="row"><dt>&#36319;&#38543;&#36317;&#31163;</dt><dd id="follow_distance">--</dd></div>
  ...
  <script>
    ...
    async function refresh() {
      try {
        ...
        if (["SEARCH_ROTATE", "SEARCH_FAILED", "OBSTACLE_TAKEOVER"].includes(data.state)) state.classList.add("warn");
        ...
        document.getElementById("follow_distance").textContent = fmt(data.follow_distance_m, " m");
...
class PersonFollowNode(Node):
    def __init__(self):
        ...
        self.declare_parameter("status_topic", "/depth_camera/person_follow_status")
        ...
        self.declare_parameter("follow_distance_m", 1.20)
        self.declare_parameter("distance_tolerance_m", 0.08)
        self.declare_parameter("follow_max_forward_mps", 0.40)
        self.declare_parameter("follow_linear_gain", 1.0)
        self.declare_parameter("follow_angular_gain", 0.8)
        self.declare_parameter("follow_max_angular_z", 0.25)
        self.declare_parameter("center_tolerance_fraction", 0.02)
        self.declare_parameter("search_angular_z", 1.25)
        self.declare_parameter("search_max_yaw_rad", 6.2831853)
        ...
        self.declare_parameter("web_port", 8093)
        ...
        self._last_output = FollowOutput(state="IDLE", reason="idle")
        ...
        self._controller = PersonFollowController(self._config())
        ...
        self.get_logger().info(
            "person follow listening color=%s depth=%s odom=%s obstacle=%s output=%s status=%s"
    ...
    def _config(self) -> PersonFollowConfig:
        return PersonFollowConfig(
            ...
            follow_max_forward_mps=float(self.get_parameter("follow_max_forward_mps").value),
            follow_linear_gain=float(self.get_parameter("follow_linear_gain").value),
            follow_angular_gain=float(self.get_parameter("follow_angular_gain").value),
            follow_max_angular_z=float(self.get_parameter("follow_max_angular_z").value),
    ...
    def _tick(self) -> None:
        ...
        if self._camera_stale(now):
            if self._obstacle_bypass_active(now):
                self._last_output = _follow_obstacle_takeover_status(self._last_output)
            elif self._controller.state not in {"IDLE", "SEARCH_FAILED"}:
                self._last_output = FollowOutput(state=self._controller.state, reason="waiting_camera")
                if _person_follow_should_publish_zero(self._last_output):
                    self._publish_zero()
        ...
        if now - self._last_process_s >= float(self.get_parameter("process_period_sec").value):
            ...
            if self._last_output.state in {"SEARCH_FAILED"}:
                self._terminal_zero_sent = False

        if now - self._last_publish_s >= float(self.get_parameter("control_period_sec").value):
            self._last_publish_s = now
            if _person_follow_should_publish(self._last_output):
                self._publish_output(self._last_output)
            elif not self._terminal_zero_sent and _person_follow_should_publish_zero(self._last_output):
                self._publish_zero()
    ...
    def _obstacle_bypass_active(self, now: float, *, target_available: bool = False) -> bool:
        ...
        if now - self._latest_obstacle_status_s > timeout:
            return False
        return _follow_obstacle_bypass_active(
    ...
    def _update_latest_jpeg(
        ...
        annotated = _draw_follow_overlay(color_bgr, detections, selected, self._last_output)
    ...
    def _publish_output(self, output: FollowOutput) -> None:
        msg = Twist()
    ...
    def _start_web_server(self) -> None:
        ...
        self._server_thread = threading.Thread(
            ...
            name="person_follow_web",
            daemon=True,
        )
        self._server_thread.start()
        self.get_logger().info("person follow web listening on http://%s:%d" % (host, port))


def _follow_obstacle_bypass_active(
    status: Optional[dict],
    *,
    target_available: bool = False,
) -> bool:
    return _seek_obstacle_bypass_active(status, target_available=target_available)


def _follow_obstacle_takeover_status(previous: FollowOutput) -> FollowOutput:
    return FollowOutput(
...
def _person_follow_should_publish(output: FollowOutput) -> bool:
    return output.state in {"SEARCH_ROTATE", "FOLLOW"} and output.reason != "obstacle_guard_takeover"


def _person_follow_should_publish_zero(output: FollowOutput) -> bool:
    if output.state in {"IDLE", "OBSTACLE_TAKEOVER"}:
        return False
    if output.reason == "obstacle_guard_takeover":
        return False
    return not _person_follow_should_publish(output)


def _follow_status_payload(
    output: FollowOutput,
    *,
    target_count: int,
    ...
    return {
        ...
        "follow_distance_m": round(float(follow_distance_m), 3),
...
def _draw_follow_overlay(
    ...
    if output.state in {"SEARCH_ROTATE", "SEARCH_FAILED", "OBSTACLE_TAKEOVER"}:
