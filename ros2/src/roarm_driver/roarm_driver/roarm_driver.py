...
def build_joint_command(names, positions):
    if not all(joint_name in names for joint_name in ARM_JOINT_NAMES):
        return None

    try:
        base = -positions[names.index('base_link_to_link1')]
        shoulder = -positions[names.index('link1_to_link2')]
        elbow = positions[names.index('link2_to_link3')]
        hand = math.pi - positions[names.index('link3_to_gripper_link')]
    except IndexError:
        return None
...
class RoarmDriver(Node):
    ...
    def __init__(self):
        ...
        try:
            ...
            self.get_logger().info(f"Connected: {serial_port_name} @ {baud_rate}")
            ...
            self.get_logger().error(f"Cannot open {serial_port_name}: {e}")
            return
        ...
        self._ignored_joint_state_warned = False

    def joint_states_callback(self, msg):
        command = build_joint_command(msg.name, msg.position)
        if command is None:
            if not self._ignored_joint_state_warned:
                self.get_logger().warning(
                    f"Ignoring non-arm /joint_states names={list(msg.name)}"
                )
                self._ignored_joint_state_warned = True
            return
...
def main(args=None):
    ...
    if roarm_driver.serial_port is not None and roarm_driver.serial_port.is_open:
        ...
        roarm_driver.get_logger().warn("Serial port not available, driver exiting")
