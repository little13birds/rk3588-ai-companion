...
OBCameraNode::OBCameraNode(rclcpp::Node* node, std::shared_ptr<ob::Device> device,
  ...
  run_save_image_thread_ = true;
  save_image_thread_ = std::make_shared<std::thread>([this]() { saveImageThread(); });
}
...
void OBCameraNode::clean() {
  ...
  run_save_image_thread_ = false;
  save_image_cv_.notify_all();
  if (save_image_thread_ && save_image_thread_->joinable()) {
    save_image_thread_->join();
  }
...
void OBCameraNode::setupDevices() {
  ...
  try {
    ...
    if (sync_mode_ != OB_SYNC_MODE_CLOSE) {
      ...
      if (device_->isPropertySupported(OB_PROP_SYNC_SIGNAL_TRIGGLE_OUT_BOOL,
                                       OB_PERMISSION_READ_WRITE)) {
        device_->setBoolProperty(OB_PROP_SYNC_SIGNAL_TRIGGLE_OUT_BOOL, sync_signal_trigger_out_);
      }
    }
...
void OBCameraNode::getParameters() {
  ...
  setAndGetNodeParameter(save_image_count_, "save_image_count", 100);
...
void OBCameraNode::onNewFrameCallback(const std::shared_ptr<ob::Frame>& frame,
  ...
  auto hardware_timestamp = frame->timeStamp();
  auto system_timestamp = frame->systemTimeStamp();
  auto index = frame->index();
  saveImageToQueue(stream_index, image, image_msg, hardware_timestamp, system_timestamp, index);
}

void OBCameraNode::saveImageToQueue(const stream_index_pair& stream_index, const cv::Mat& image,
                                    const sensor_msgs::msg::Image::SharedPtr& image_msg,
                                    uint64_t hardware_timestamp, uint64_t system_timestamp,
                                    uint64_t index) {
  if (save_images_[stream_index] && remaining_images_to_save_[stream_index] > 0) {
    auto current_path = std::filesystem::current_path().string();
    auto fps = fps_[stream_index];
    auto serial_number = device_->getDeviceInfo()->serialNumber();
    std::string image_path = current_path + "/image/" + serial_number + "/";
    std::string filename = image_path + stream_name_[stream_index] + "_" + std::to_string(index) +
                           "_" + std::to_string(system_timestamp) + "_" +
                           std::to_string(hardware_timestamp) + "_" +
                           std::to_string(image_msg->width) + "x" +
                           std::to_string(image_msg->height) + "_" + std::to_string(fps) + "hz.png";
    if (!std::filesystem::exists(image_path)) {
      std::filesystem::create_directory(image_path);
    }
    bool is_empty_before = image_queue_.empty();
    if (stream_index.first == OB_STREAM_DEPTH) {
      auto image_to_save = cv_bridge::toCvCopy(image_msg, encoding_[stream_index])->image;
      image_queue_.emplace(filename, image_to_save);
      ...
      image_queue_.emplace(filename, image);
    } else {
      RCLCPP_ERROR_STREAM(logger_, "Unsupported stream type: " << stream_index.first);
    }
    if (is_empty_before && !image_queue_.empty()) {
      save_image_cv_.notify_one();
    }
    remaining_images_to_save_[stream_index]--;
  } else {
    save_images_[stream_index] = false;
  }
}

void OBCameraNode::saveImageThread() {
  while (run_save_image_thread_) {
    std::queue<std::pair<std::string, cv::Mat>> local_image_queue;
    {
      std::unique_lock<std::mutex> lock(save_image_mutex_);
      save_image_cv_.wait(lock,
                          [this] { return !image_queue_.empty() || !run_save_image_thread_; });
      if (!run_save_image_thread_) {
        return;
      }

      std::swap(local_image_queue, image_queue_);
    }

    while (!local_image_queue.empty()) {
      auto image_to_save = local_image_queue.front();
      local_image_queue.pop();
      cv::imwrite(image_to_save.first, image_to_save.second);
    }
  }
}
